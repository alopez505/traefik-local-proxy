...
Prerequisites: Docker Engine with Compose v2, `mise`, and `just`.

Choose one startup mode:

| Mode | Command | Dashboard URL | Needs generated certs? | Needs Windows trust? |
| --- | --- | --- | --- | --- |
| HTTPS (default, closest to production) | `just up` | <https://proxy.localtest.me> | Yes (`certificates-generate`) | Yes (`certificates-trust-ca`) |
| HTTP-only (no local CA required) | `just up-http` | <http://proxy.localtest.me> | No | No |

### HTTPS mode (default)

```bash
mise trust                      # one time per clone
mise install                    # install pinned tools (just, mkcert, jq, uv)
cp .env.example .env            # optional: override ports or log level
just certificates-generate      # generate the wildcard cert via mkcert (skips if present)
just certificates-trust-ca      # import the CA into Windows CurrentUser\Root (WSL2 -> Windows)
just up                         # start Traefik
just demo                       # start whoami test service -> https://demo.localtest.me
just demo-down                  # remove the demo container after testing
```
...
`just certificates-trust-ca` imports that CA into the Windows trust store so
...
optional: run `just up-http` without generating or trusting certificates.
...
ports. Running `just up-http` while HTTPS is running, or `just up` while
...
change their labels. Use `just demo-http` for the HTTP demo, or update an
...
| Recipe | Purpose |
| --- | --- |
| `just up` / `just up-http` | Start Traefik in HTTPS or HTTP-only mode |
| `just down` / `just stop` | Remove or stop the proxy |
| `just certificates-generate` | Generate the wildcard cert via mkcert |
| `just certificates-trust-ca` | Import the dev CA into the Windows trust store |
| `just demo` / `just demo-down` | Start/stop the whoami test service |
| `just validate` | Full local gate: pre-commit hooks + all compose configs |
| `just network` | List containers currently attached to the proxy network |

Full reference (all 24 recipes): [docs/operations/tasks.md](docs/operations/tasks.md).

## Layout

```text
dynamic/           TLS config, backend-TLS transport, optional hardening middlewares
examples/whoami/   Minimal test service used by `just demo`
